...
var testTimezone = '';
gulp.task('karma:unit', gulp.series('ng:test/html', function() {
...
gulp.task('karma:server', gulp.series('ng:test/html', function karmaServer() {
  karma.start({
    configFile: path.join(__dirname, 'test/karma.conf.js'),
    browsers: ['PhantomJS'],
    reporters: ['progress'],
...
gulp.task('karma:travis', gulp.series('ng:test/html', function karmaTravis() {
  karma.start({
    configFile: path.join(__dirname, 'test/karma.conf.js'),
    browsers: ['PhantomJS'],
...
gulp.task('karma:travis~1.2.0', gulp.series('ng:test/html', function karmaTravis120() {
  karma.start({
...
gulp.task('karma:travis~1.4.0', gulp.series('ng:test/html', function karmaTravis140() {
  karma.start({
...
gulp.task('test', gulp.series('ng:test/html', gulp.parallel('jshint', 'karma:unit')));
gulp.task('test:timezone', function() {
  ...
  return gulp.series('ng:test/html', gulp.parallel('jshint', 'karma:unit'));
});
gulp.task('test:server', gulp.series('ng:test/html', 'karma:server'));
